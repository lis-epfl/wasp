...
def ini_rf_reciever():
    '''
    Initialize the CC1101 as reciever to match the signal sent by the HT Keyfob transmitter.
    '''
    # --- SPI Setup ---
    spi = spidev.SpiDev()
    ...
    CC1101_CONFIG = {
        ...
        0x10: 0xC7,  # MDMCFG4       Modem Configuration
        0x11: 0x93,  # MDMCFG3       Modem Configuration
...
BIT_DURATION = 416.7  # microseconds
...
print("Listening for GPIO transitions... Ctrl+C to stop")
...
try:
    while True:
        ...
        if event:
            ...
            # Check if edge type changed (i.e., signal level stayed constant before)
            if last_tick_ns is not None and evt.type != last_event_type:
                ...
                if bit_count > 0:
                    ...
                    print(bits_to_add, end='', flush=True)

            # Append current edge bit
            #buffer.append(str(level))
            #print(str(level), end='', flush=True)

            # Update last tick, level, and event type
            ...
            # Sequence detection
            # Optional: Only print if bit is part of sequence
            if len(buffer) >= SEQUENCE_SIZE:
                current_seq = ''.join(buffer)
                #print(counter, "cur:",current_seq, len(current_seq))

                for idx, seq in enumerate(sequences, 1):
                    #print(counter, "seq:",seq, len(seq))
                    if current_seq == seq:

                        print(f"\nSequence S{idx} detected!")
